...
# -*- coding: utf-8 -*-
import rospy
from std_msgs.msg import Float32MultiArray, Header, Float64
...
class MoraiAIBridge:
    def __init__(self):
        self.node_name = 'morai_ai_bridge'
        ...
        try:
            ...
            rospy.loginfo("%s: Successfully bound to %s:%d for VehicleStatus",
                        self.node_name, self.user_ip, self.vehicle_status_port)
        except Exception as e:
            rospy.logerr("%s: Failed to bind to %s:%d - Error: %s",
                        self.node_name, self.user_ip, self.vehicle_status_port, str(e))
            rospy.logerr("Port 907 requires root/sudo privileges. Try: sudo -E rosrun ...")
            raise
        ...
        self.heading_pub = rospy.Publisher('/Local/heading', Float64, queue_size=10)
        self.position_pub = rospy.Publisher('/fix', PointStamped, queue_size=1)
    ...
    def initialize_data(self):
        ...
        self.pitch_r_ = 0.0
        self.yaw_r_ = 0.0
        self.ang_vel_x_r_ = 0.0
        self.ang_vel_y_r_ = 0.0
        self.ang_vel_z_r_ = 0.0
        self.accel_x_r_ = 0.0
        self.accel_y_r_ = 0.0
        self.accel_z_r_ = 0.0
        self.orientation_w_r_ = 0.0
        self.orientation_x_r_ = 0.0
        self.orientation_y_r_ = 0.0
        self.orientation_z_r_ = 0.0
    ...
    def write_ctrl_cmd(self):
        # ctrl_mode/gear 유지
        self.ctrl_cmd_data.ctrl_mode = 0
    ...
    def read_vehicle_status(self):
        ...
        self.pos_x_r_ = status.pos_x
        self.pos_y_r_ = status.pos_y
        self.pos_z_r_ = status.pos_z

        self.roll_r_ = status.roll * (math.pi / 180.0)  # deg to rad
        self.pitch_r_ = status.pitch * (math.pi / 180.0)  # deg to rad
        self.yaw_r_ = status.yaw * (math.pi / 180.0)  # deg to rad
        self.ang_vel_x_r_ = status.ang_vel_x * (math.pi / 180.0)  # deg to rad
        self.ang_vel_y_r_ = status.ang_vel_y * (math.pi / 180.0)  # deg to rad
        self.ang_vel_z_r_ = status.ang_vel_z * (math.pi / 180.0)  # deg to rad
        self.accel_x_r_ = status.accel_x
        self.accel_y_r_ = status.accel_y
        self.accel_z_r_ = status.accel_z

        quat = self.euler_to_quaternion(self.roll_r_, self.pitch_r_, self.yaw_r_)
        self.orientation_x_r_ = quat[0]  # x
        self.orientation_y_r_ = quat[1]  # y
        self.orientation_z_r_ = quat[2]  # z
        self.orientation_w_r_ = quat[3]  # w

        msg_fix = PointStamped()
        msg_fix.header.stamp = rospy.Time.now()
        msg_fix.header.frame_id = 'map'
        msg_fix.point.x = self.pos_x_r_
        msg_fix.point.y = self.pos_y_r_
        msg_fix.point.z = self.pos_z_r_
        # 위치 정보 퍼블리시
        self.position_pub.publish(msg_fix)

        msg_erp = Float32MultiArray()
        msg_erp.data = [
        ...
        self.erp_pub.publish(msg_erp)
        rospy.loginfo("Recv Status: mode=%.0f, gear=%.0f, vel=%.2f m/s, steer=%.3f rad, brake=%.3f",
                      msg_erp.data[0], msg_erp.data[2], msg_erp.data[3], msg_erp.data[4], msg_erp.data[5])
        
        msg_imu = Imu()
        msg_imu.header.stamp = rospy.Time.now()
        msg_imu.header.frame_id = 'imu'

        msg_imu.orientation.w = self.orientation_w_r_
        msg_imu.orientation.x = self.orientation_x_r_
        msg_imu.orientation.y = self.orientation_y_r_
        msg_imu.orientation.z = self.orientation_z_r_

        msg_imu.angular_velocity.x = self.ang_vel_x_r_
        msg_imu.angular_velocity.y = self.ang_vel_y_r_
        msg_imu.angular_velocity.z = self.ang_vel_z_r_

        msg_imu.linear_acceleration.x = self.accel_x_r_
        msg_imu.linear_acceleration.y = self.accel_y_r_
        msg_imu.linear_acceleration.z = self.accel_z_r_

        self.imu_pub.publish(msg_imu)

        msg_heading = Float64()
        msg_heading.data = self.yaw_r_
